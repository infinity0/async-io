...
impl RLBuf {
  ...
  */
  pub fn add_allowance(&mut self, allowance: usize) {
    ...
    // TODO: wake up the future, if any
  }
...
/** Rate-limited asynchronous analogue of `std::io::BufReader` */
#[derive(Debug)]
pub struct RLRead<T> {
    raw: T,
    rstatus: Status,
...
impl<T> RLRead<T> where T: Read {
  /** Create a new `RLRead` with the given initial capacity.

  The raw stream must already be in non-blocking mode.
  */
  pub fn new_lb(raw: T, init: usize) -> RLRead<T> {
    RLRead {
      raw: raw,
      rstatus: SOpen,
  ...
  pub fn pre_read(&mut self) {
    match self.rstatus {
      SOpen => {
        ...
        let remain = self.rbuf.get_demand_cap() - self.rbuf.get_demand();
        let mut buf = [0].repeat(remain); // TODO: optimise with uninit
        match self.raw.read(&mut buf) { // TODO: assert non-blocking
          Ok(0) => {
            self.rstatus = SOk
          },
          Ok(n) => {
...
impl<T> Read for RLRead<T> where T: Read {
  fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
    match self.rbuf.get_demand() {
      0 => match self.rstatus {
        SOpen => Err(io::Error::new(ErrorKind::WouldBlock, "")),
        SOk => Ok(0),
        SErr => Err(unwrap_err_or(self.raw.read(&mut []), io::Error::new(ErrorKind::Other, "Ok after Err"))),
      },
...
/// This trait exists so you can wrap both `RLRead` and `RLWrite` around a rw
/// handle such as `TcpStream`.
impl<T> Write for RLRead<T> where T: Write {
...
/** Rate-limited asynchronous analogue of `std::io::BufWriter`
...
*/
#[derive(Debug)]
pub struct RLWrite<T> {
    raw: T,
    wstatus: Status,
    wbuf: RLBuf,
}

impl<T> RLWrite<T> where T: Write {
  /** Create a new `RLWrite` with the given initial capacity.

  The raw stream must already be in non-blocking mode.
  */
  pub fn new_lb(raw: T, init: usize) -> RLWrite<T> {
    RLWrite {
      raw: raw,
      wstatus: SOpen,
...
impl<T> Write for RLWrite<T> where T: Write {
  fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
    match self.wstatus {
      SOpen => {
        // TODO: if allowance is 0, then automatically grow the buffer capacity
        let remain = self.wbuf.get_demand_cap() - self.wbuf.get_demand();
        match remain {
          0 => Err(io::Error::new(ErrorKind::WouldBlock, "")),
          _ => {
      ...
      SOk => Ok(0),
      SErr => Err(unwrap_err_or(self.raw.write(&mut []), io::Error::new(ErrorKind::Other, "Ok after Err"))),
    }
  }
  ...
  fn flush(&mut self) -> io::Result<()> {
    match self.wstatus {
      SErr =>
        ...
        Err(unwrap_err_or(self.raw.write(&mut []), io::Error::new(ErrorKind::Other, "Ok after Err"))),
...
/// This trait exists so you can wrap both `RLRead` and `RLWrite` around a rw
/// handle such as `TcpStream`.
impl<T> Read for RLWrite<T> where T: Read {
  fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
    self.raw.read(buf)
  }
}
...
mod tests {
  ...
  fn read_eof_ok() -> io::Result<()> {
    ...
    let mut bf = RLRead::new_lb(file, 1);
  ...
  fn read_zero_err() -> io::Result<()> {
    ...
    let sx = 1024;
    let sy = 1024;
    let mut bf = RLRead::new_lb(file, sd);
  ...
  fn write_eof_err() -> io::Result<()> {
    let file = File::open("/dev/zero")?;
    set_non_blocking(file.as_raw_source())?;
    let mut bf = RLWrite::new_lb(file, 1);
    let buf = [0].repeat(1);
  ...
  fn write_null_ok() -> io::Result<()> {
    ...
    let sx = 1024;
    let sy = 1024;
    let mut bf = RLWrite::new_lb(file, sd);
